...
export class SegmentoPagoDialogoComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) private data : SegmentoCredito,
  ...
  ngOnInit(): void {
    this.segmentoNombre = this.data.segmento.nombre;
    this.dataSourceFacturasAPagar = new MatTableDataSource();
    this.segmentoCreditoDetalleService.getBySegmentoSinPagar(this.data.segmento.idSegmento).subscribe(data => {
